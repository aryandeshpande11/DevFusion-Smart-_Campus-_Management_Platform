...
import { getMySubmissions, submitAssignmentSolution } from "../../../api/assignmentApi.js";
import { useToast } from "../../../components/common/Toast.jsx";

const statusTone = { pending: "warning", reviewed: "success" };

// list of assignments assigned to the student, each with a submit action
export default function StudentAssignments() {
  const { data: submissions, isLoading, reload } = useFetch(getMySubmissions);
        ...
        {submissions?.map((item) => (
          <Card key={item.id} className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted">Due {item.deadline}</p>
            </div>
